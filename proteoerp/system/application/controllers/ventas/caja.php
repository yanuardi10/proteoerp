...
class Caja extends validaciones {
	...
	function filteredgrid(){
		...
		$grid->column("Descontar de almacen", "almacen");
		$grid->column("Impresora Puerto","impre");
		$grid->column("Lector Optico Puerto","lector");
	...
	function dataedit(){ 
		...
		$edit->ubica->append('Puede colocar la direccion IP de la caja');
		$edit->ubica->maxlength=30;
		$edit->ubica->rule="trim|strtoupper|callback_ipcaja";
		
		$edit->status = new dropdownField("Status", "status");
		$edit->status->option("","");
		$edit->status->options(array("A"=> "Abierto","C"=>"Cerrado"));
