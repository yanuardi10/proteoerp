...
class Ganancias extends Controller {  
 ...
 function Ganancias() {
	...
	function linea($anio='',$departamento=''){
		...
		if(empty($anio) OR empty($departamento)) redirect("ventas/ganancias/departamento/$anio");
		...
		$filter = new DataForm('ventas/ganancias/linea');
		$filter->title('Filtro de Ganancias');
		...
		$filter->button("btnsubmit", "Buscar", form2uri(site_url('ventas/ganancias/linea/'),array('anio','depto')), $position="BL");
		$filter->build_form();
		...
		$grafico = open_flash_chart_object(680,450, site_url("ventas/ganancias/glinea/$anio/$departamento"));
		$data['content']  =$grafico;
		$data['content'] .= $filter->output.$grid->output;
		$data["head"]     = $this->rapyd->get_head();
		$data['title']    = $this->rapyd->get_head()."<h1>Ganancias por Lineas</h1>";
		$this->load->view('view_ventanas', $data);
	}
    ...
    $ajax_onchange = '
	  ...
	  ';
		...
		if(empty($anio) AND empty($departamento)) redirect('ventas/ganancias/departamento/');
    if(empty($linea)) redirect("ventas/ganancias/linea/$anio/$departamento");

		$fechai=$anio.'0101';
		$fechaf=$anio.'1231';

		$filter = new DataForm('ventas/ganancias/grupo');
    ...
    $filter->departamento->onchange = "get_linea();";
		...
		$filter->button("btnsubmit", "Buscar", form2uri(site_url('ventas/ganancias/grupo/'),array('anio','depto','linea')), $position="BL");
		$filter->build_form();
  	...
  	$grid->column("Grupo"      , "nombre","align='left'");
		...
		$grafico = open_flash_chart_object(680,450, site_url("ventas/ganancias/ggrupo/$anio/$departamento/$linea"));
		$data['content']  =$grafico;
		$data['content'] .=  $filter->output.$grid->output;
		$data["head"]     = $this->rapyd->get_head();
    ...
    $ajax_onchange = '
	  ...
	  ';
		...
		if(empty($linea)) redirect("ventas/ganancias/linea/$anio/$departamento");
		if(empty($grupo)) redirect("ventas/ganancias/grupo/$anio/$departamento/$linea");
	...
	function gdepartamento($anio=''){
		...
		for($i=0;$i<count($data_1);$i++ ){
			...
			$bar_1->links[]= site_url("/ventas/ganancias/linea/$anio/".$departamento[$i]);		 	                                                                                  
		} 			 
	...
	function glinea($anio='',$departamento=''){
		...
		GROUP BY d.linea ORDER BY ganancias DESC LIMIT 15";   
		//echo $mSQL;
		...
		for($i=0;$i<count($data_1);$i++ ){
			...
			$bar_1->add_data_tip($data_1[$i]/$om, graph::esc( number_format($data_1[$i],2,',','.')));
			$bar_1->links[]= site_url("ventas/ganancias/grupo/$anio/$departamento/".$linea[$i]);
		...
		if($maxval>0){
		$g->title( 'Ganancias por lineas del departamento de '.$ndepartamento.' en el a&ntilde;o '.$anio,'{font-size: 16px; color:#0F3054}' ); 
		$g->data_sets[] = $bar_1;
    ...
    $mSQL.=" WHERE a.origen IN ('3I','3M') AND a.fecha>='$fechai' AND a.fecha<='$fechaf'AND e.depto='$departamento'AND d.linea='$linea'
		...
		for($i=0;$i<count($data_1);$i++ ){
			...
			$bar_1->add_data_tip($data_1[$i]/$om, graph::esc( number_format($data_1[$i],2,',','.')));
			$bar_1->links[]= site_url("ventas/ganancias/producto/$anio/$departamento/$linea/".$grupo[$i]);
		} 			 
		...
		if($maxval>0){
		$g->title('Ganancias por grupo de la linea '.$nlinea.' en el a&ntilde;o '.$anio,'{font-size: 16px; color:#0F3054}' ); 
		$g->data_sets[] = $bar_1;
