...
class add extends Controller {
	...
	function index() {	
		redirect('compras/add/encab/create');
	}
	...
	function mdetalle($control){
      ...
      if(isset($_POST[$obj]))$data['value']=$_POST[$obj];
		...
		$edit->buttons("save", "undo", "delete", "back");
		$edit->build();
		$jqlink=site_url('compras/agregar/aplicaiva');
	...
	function aplicaiva(){
		...
		$fivas=$this->datasis->ivaplica($mfecha);
	...
	function _post_minsert($do){
		...
		$mSQL="SELECT SUM(a.importe) AS montotot, ROUND(SUM(importe*(a.iva/100)),2) AS montoiva , ROUND(SUM(b.peso),2) AS peso FROM itscst AS a JOIN sinv AS b ON a.codigo=b.codigo WHERE control='$control'";
		...
		if(eregi('ESPECIAL', $tcontri)){
			...
			$reteiva=$this->datasis->dameval($mSQL);
			$reteiva= ($reteiva>0) ? $reteiva : 75;
			$reteiva=$reteiva/100;
	...
	function _post_mdelete($do){
		$this->_post_minsert($do);
	}
