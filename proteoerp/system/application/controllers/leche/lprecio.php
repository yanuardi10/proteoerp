<?php
class Lprecio extends Controller {
	var $mModulo = 'LPRECIO';
	...
	function Lprecio(){
		...
		$this->datasis->modulo_nombre( 'LPRECIO', $ventana=0 );
	}

	function index(){
		$this->datasis->creaintramenu(array('modulo'=>'228','titulo'=>'Precios','mensaje'=>'Precios','panel'=>'LECHE','ejecutar'=>'leche/lprecio','target'=>'popu','visible'=>'S','pertenece'=>'2','ancho'=>900,'alto'=>600));
	...
	function jqdatag(){
		...
		$param['listados']    = $this->datasis->listados('LPRECIO', 'JQ');
		$param['otros']       = $this->datasis->otros('LPRECIO', 'JQ');
					...
					success: function(r,s,x){
						try{
							...
							if (json.status == "A"){
								...
								'.$this->datasis->jwinopen(site_url('formatos/ver/LPRECIO').'/\'+res.id+\'/id\'').';
								return true;
							} else {
	...
	function defgrid( $deployed = false ){
		...
		$grid->addField('tarifa5');
		$grid->label('Machique F');
		$grid->params(array(
			'search'        => 'true',
			'editable'      => $editar,
			'align'         => "'right'",
		...
		$grid->addField('tarifa6');
		$grid->label('Machique C');
		$grid->params(array(
			'search'        => 'true',
			'editable'      => $editar,
			'align'         => "'right'",
		...
		$grid->setBarOptions("\t\taddfunc: lprecioadd,\n\t\teditfunc: lprecioedit");
	...
	*/
	function getdata()
	{
		...
		$mWHERE = $grid->geneTopWhere('lprecio');

		$response   = $grid->getData('lprecio', array(array()), array(), false, $mWHERE );
	...
	*/
	function setData()
	{
		...
		if($oper == 'add'){
			if(false == empty($data)){
				$check = $this->datasis->dameval("SELECT count(*) FROM lprecio WHERE $mcodp=".$this->db->escape($data[$mcodp]));
				if ( $check == 0 ){
					$this->db->insert('lprecio', $data);
					echo "Registro Agregado";

					logusu('LPRECIO',"Registro ????? INCLUIDO");
				} else
			...
			$nuevo  = $data[$mcodp];
			$anterior = $this->datasis->dameval("SELECT $mcodp FROM lprecio WHERE id=$id");
			if ( $nuevo <> $anterior ){
				//si no son iguales borra el que existe y cambia
				$this->db->query("DELETE FROM lprecio WHERE $mcodp=?", array($mcodp));
				$this->db->query("UPDATE lprecio SET $mcodp=? WHERE $mcodp=?", array( $nuevo, $anterior ));
				$this->db->where("id", $id);
				$this->db->update("lprecio", $data);
				logusu('LPRECIO',"$mcodp Cambiado/Fusionado Nuevo:".$nuevo." Anterior: ".$anterior." MODIFICADO");
				echo "Grupo Cambiado/Fusionado en clientes";
			} else {
				unset($data[$mcodp]);
				$this->db->where("id", $id);
				$this->db->update('lprecio', $data);
				logusu('LPRECIO',"Grupo de Cliente  ".$nuevo." MODIFICADO");
				echo "$mcodp Modificado";
			}

		} elseif($oper == 'del') {
			$meco = $this->datasis->dameval("SELECT $mcodp FROM lprecio WHERE id=$id");
			//$check =  $this->datasis->dameval("SELECT COUNT(*) FROM lprecio WHERE id='$id' ");
			if ($check > 0){
				echo " El registro no puede ser eliminado; tiene movimiento ";
			} else {
				$this->db->simple_query("DELETE FROM lprecio WHERE id=$id ");
				logusu('LPRECIO',"Registro ????? ELIMINADO");
