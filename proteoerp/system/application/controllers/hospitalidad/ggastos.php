...
class GGastos extends Controller {  
	...
	function index(){
		redirect('/hospitalidad/ggastos/anuales');
	}
	...
	function anuales(){
		...
		$filter = new DataForm('hospitalidad/ggastos/anuales');
		...
		$filter->button("btnsubmit", "Buscar", form2uri(site_url('hospitalidad/ggastos/anuales/'),array('anio')), $position="BL");
		$filter->build_form();
    ...
    "COUNT(*) AS numfac");  
		...
		$grafico = open_flash_chart_object(680,450, site_url("hospitalidad/ggastos/ganuales/$anio/"));
	...
	function mensuales($anio='',$proveed=''){
		...
		if(empty($anio) OR empty($proveed)) redirect("hospitalidad/ggastos/anuales/$anio");
		...
		$filter = new DataForm('ventas/gastos/mensuales');
		...
		$filter->button("btnsubmit", "Buscar", form2uri(site_url('hospitalidad/ggastos/mensuales/'),array('anio','proveed')), $position="BL");
		$filter->build_form();
    ...
    "COUNT(*) AS numfac");
		...
		$grafico = open_flash_chart_object(750,350, site_url("/hospitalidad/ggastos/gmensuales/$anio/".str_replace('/',':slach:',$proveed)));
		$data['content']  =$grafico;
		$data['content'] .= $filter->output.$grid->output;
		$data["head"]     = $this->rapyd->get_head();
	...
	function diarias ($anio='',$proveed='',$mes=''){
		...
		if(empty($mes))redirect("hospitalidad/ggastos/mensuales/$anio/$proveed");
		...
		$filter = new DataForm('hospitalidad/ggastos/diarias');
		...
		$filter->button("btnsubmit", "Buscar", form2uri(site_url('hospitalidad/ggastos/diarias/'),array('anio','proveed','mes')), $position="BL");
		$filter->build_form();
    ...
    "COUNT(*) AS numfac");
		...
		$grafico = open_flash_chart_object(680,350, site_url("hospitalidad/ggastos/gdiarias/$anio/$proveed/$mes"));
		$data['content']  =$grafico;
		$data['content'] .=  $filter->output.$grid->output;
		$data["head"]     = $this->rapyd->get_head();
    ...
    GROUP BY proveed ORDER BY grantotal DESC LIMIT 10";
		...
		for($i=0;$i<count($data_1);$i++ ){
		  ...
		  $bar_1->links[]= site_url("/hospitalidad/ggastos/mensuales/$anio/".str_replace('/',':slach:',$proveed[$i]));		                                                                                  
		} 			 
	...
	function gmensuales($anio='',$proveed=''){
		...
		$mSQL = "SELECT fecha,proveed,nombre,MONTHNAME(fecha) AS mes,
		SUM(totneto)AS grantotal
		FROM gser
		...
		for($i=0;$i<count($data_1);$i++ ){
			$bar_1->add_data_tip($data_1[$i]/$om, graph::esc( number_format($data_1[$i],2,',','.')));
			$mes=$i+1;
		  $bar_1->links[]= site_url("/hospitalidad/ggastos/diarias/$anio/$proveed/$mes");
