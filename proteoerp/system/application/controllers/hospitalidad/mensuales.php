...
class Mensuales extends Controller {
	...
	function index() {
		...
		if(empty($mes)OR empty($anio))$mes=date("n");$anio= date("Y"); 
	...
	function _sincrodata($fechai,$fechaf){
		...
		COUNT(*) AS numfac FROM sfac WHERE tipo<>'X' AND fecha>='$fechai' AND fecha<='$fechaf' GROUP BY fecha";
		
		$mSQL_2="SELECT fecha, DATE_FORMAT(fecha,'%d') AS dia , SUM(monto) AS cobrado FROM sfpa 
		WHERE tipo<>'X' AND fecha>='$fechai' AND fecha<='$fechaf' AND tipo='AB'
