...
class Libros_calore extends Controller {
	...
	function index() {
		...
		$submit= form_submit('<#accion#>', 'Generar');; $sanio= 
		form_dropdown('year',$anhos,date('Y')); $smes = 
		form_dropdown('mes',$mmes,date('m'));
		...
		$link=site_url('finanzas/libros_calore/generar');
		...
		function descarga(nombre){
			...
			window.location="'.site_url('finanzas/libros_calore').'/"+nombre+"/"+param;
			//alert(param);
		}
		</script>';
		$conf=anchor('finanzas/libros_calore/configurar','Configurar');
		$form=  form_open('/finanzas/libros_calore');
	...
	function wlcexcel($mes){
      ...
      CONCAT(EXTRACT(YEAR_MONTH FROM fechal),b.nrocomp) nrocomp,
	    ...
	    sum(a.stotal   *IF(a.tipo='NC',-1,1)) stotal
      ...
      WHERE libro='C' AND EXTRACT(YEAR_MONTH FROM fechal) ='$mes' AND a.fecha>0 AND a.reiva>0 
		...
		$ws->write_string( $mm+1, 4, "NºControl", $titulo );
		$ws->write_string( $mm+2, 4, "", $titulo );
		$ws->write_string( $mm,   5, "Numero de",    $titulo );
		$ws->write_string( $mm+1, 5, "Declaracion",  $titulo );
		$ws->write_string( $mm+2, 5, "de Aduanas",   $titulo );
		$ws->write_string( $mm,    6, "Nombre, Razon Social", $titulo );
		$ws->write_string( $mm+1,  6, "o", $titulo );
		$ws->write_string( $mm+2,  6, "Denominacion del Proveedor", $titulo );
		$ws->write_string( $mm,    7, "Numero", $titulo );
		$ws->write_string( $mm+1,  7, "de", $titulo );
		$ws->write_string( $mm+2,  7, "R.I.F.", $titulo );
		$ws->write_string( $mm,    8, "Total Compras", $titulo );
		$ws->write_string( $mm+1,  8, "incluyendo", $titulo );
		$ws->write_string( $mm+2,  8, "el I.V.A.", $titulo );
		$ws->write_string( $mm,    9, "Compras No", $titulo );
		$ws->write_string( $mm+1,  9, "Gravadas o sin", $titulo );
		$ws->write_string( $mm+2,  9, "derecho a C.F.", $titulo );
		$ws->write_string( $mm,    10, "Compras de Importacion Gravadas", $titulo );
		$ws->write_string( $mm+1,  10, "Alicuota General $tasa %", $titulo );
		$ws->write_string( $mm+2,  10, "Base", $titulo );
		$ws->write_blank( $mm,    11, $titulo );
		$ws->write_blank( $mm+1,  11, $titulo );
		$ws->write_string( $mm+2, 11, "Impuesto", $titulo );
		$ws->write_blank( $mm,    12, $titulo );
		$ws->write_string( $mm+1, 12, "Alicuota Adicional $sobretasa %",$titulo );
		$ws->write_string( $mm+2, 12, "Base", $titulo );
		$ws->write_blank( $mm,    13, $titulo );
		$ws->write_blank( $mm+1,  13, $titulo );
		$ws->write_string( $mm+2, 13, "Impuesto", $titulo );
		$ws->write_blank( $mm,    14, $titulo );
		$ws->write_string( $mm+1, 14, "Alicuota Reducida $redutasa %",$titulo );
		$ws->write_string( $mm+2, 14, "Base", $titulo );
		$ws->write_blank( $mm,    15, $titulo );
		$ws->write_blank( $mm+1,  15, $titulo );
		$ws->write_string( $mm+2, 15, "Impuesto", $titulo );
		$ws->write_string( $mm,   16, "Compras Internas Gravadas o con derecho a Credito Fiscal", $titulo );
		$ws->write_string( $mm+1, 16, "Alicuota General $tasa %", $titulo );
		$ws->write_string( $mm+2, 16, "Base", $titulo );
		$ws->write_blank( $mm,    17, $titulo );
		$ws->write_blank( $mm+1,  17, $titulo );
		$ws->write_string( $mm+2, 17, "Impuesto", $titulo );
		$ws->write_blank( $mm,    18, $titulo );
		$ws->write_string( $mm+1, 18, "Alicuota Adicional $sobretasa %",$titulo );
		$ws->write_string( $mm+2, 18, "Base", $titulo );
		$ws->write_blank( $mm,    19, $titulo );
		$ws->write_blank( $mm+1,  19, $titulo );
		$ws->write_string( $mm+2, 19, "Impuesto", $titulo );
		$ws->write_blank( $mm,    20, $titulo );
		$ws->write_string( $mm+1, 20, "Alicuota Reducida $redutasa %",$titulo );
		$ws->write_string( $mm+2, 20, "Base", $titulo );
		$ws->write_blank( $mm,    21, $titulo );
		$ws->write_blank( $mm+1,  21, $titulo );
		$ws->write_string( $mm+2, 21, "Impuesto", $titulo );
		$ws->write_string( $mm,   22, "Ajuste a los", $titulo );
		$ws->write_string( $mm+1, 22, "Creditos F.", $titulo );
		$ws->write_string( $mm+2, 22, "P. Anteriores", $titulo );
		$ws->write_string( $mm,   23, "I.V.A.", $titulo );
		$ws->write_string( $mm+1, 23, "Retenido al", $titulo );
		$ws->write_string( $mm+2, 23, "Vendedor", $titulo );
		$ws->write_string( $mm,   24, "Numero", $titulo );
		$ws->write_string( $mm+1, 24, "de", $titulo );
		$ws->write_string( $mm+2, 24, "Comprobante", $titulo );
		...
		if ( $mc->num_rows() > 0 ) {
			foreach( $mc->result() as $row ) {
				...
				$ws->write_string( $mm,  4, $row->nfiscal,  $cuerpo );
				$ws->write_string( $mm,  6, $row->nombre,  $cuerpo ); 
				$ws->write_string( $mm,  7, $row->rif,  $cuerpo );

				if ($row->oper == '05' ){
					$ws->write_number( $mm,  8, $row->gtotal, $numero );
					$ws->write_number( $mm,  9, $row->exento, $numero );
					$ws->write_number( $mm, 16, 0, $numero );
					$ws->write_number( $mm, 17, 0, $numero );
					$ws->write_number( $mm, 19, 0, $numero );
					$ws->write_number( $mm, 19, 0, $numero );
					$ws->write_number( $mm, 20, 0, $numero );
					$ws->write_number( $mm, 21, 0, $numero );
					$ws->write_number( $mm, 22, $row->gtotal-$row->stotal, $numero );
				} elseif ($row->oper == '04' ){
					$ws->write_number( $mm,  8, $row->gtotal, $numero );
					$ws->write_number( $mm,  9, $row->exento, $numero );
					$ws->write_number( $mm, 16, 0, $numero );
					...
					$ws->write_number( $mm, 20, 0, $numero );
					$ws->write_number( $mm, 21, 0, $numero );
					$ws->write_number( $mm, 22, $row->reduimpu+$row->geneimpu+$row->adicimpu, $numero );
				}else{
					$ws->write_number( $mm,  8, $row->gtotal,    $numero );
					$ws->write_number( $mm,  9, $row->exento,    $numero );
					$ws->write_number( $mm, 16, $row->general,   $numero );
				...
				$ws->write_number( $mm, 23, $row->reiva, $numero );
				$ws->write_string( $mm, 24, $row->nrocomp, $cuerpo );
				$mm++;
				$ii++;
			}
		}
	...
	function wlvexcel($mes) {
		...
		$mSQL  = "SELECT 
				...
				a.contribu, a.registro, a.comprobante, a.fecharece,b.tiva
			FROM siva a LEFT JOIN scli b ON a.clipro=b.cliente
		...
		$mSQL  = "SELECT 
			...
			SELECT 
				...
				sum(a.reducida*IF(a.tipo='NC',-1,1))  reducida,
				sum(a.reduimpu*IF(a.tipo='NC',-1,1))  reduimpu,
				'NO' contribu, '01' registro, ' ' comprobante, null fecharece,b.tiva
			FROM siva a LEFT JOIN scli b ON a.clipro=b.cliente
		...
		*/
		...
		$ws->write_string( $mm,  $mcel, "VENTAS GRAVADAS A CONTRIBUYENTES", $titulo );
		...
		$ws->write_string( $mm,  $mcel, "VENTAS GRAVADAS A NO CONTRIBUYENTES", $titulo );
		$ws->write_string( $mm+1,$mcel, "ALICUOTA GENERAL $tasa%", $titulo );
		$ws->write_string( $mm+2,$mcel, "Base", $titulo );
		$mcel++;
		...
		if ( $export->num_rows() > 0 ){
			foreach( $export->result() as $row ){
				...
				if ( $row->registro=='04' ) {
					...
					//$ws->write_number( $mm,23, $row->geneimpu + $row->adicimpu+$row->reduimpu, $numero );		// REDUIMPU
				} else {
					...
					//$ws->write_number( $mm,23, 0, $numero );		// REDUIMPU
				}
			if ( $row->tiva=='C' ){
					$ws->write_number( $mm,11, $row->general, $numero );		// GENERAL
					$ws->write_number( $mm,12, $row->geneimpu, $numero );		// GENEIMPU
					...
					$ws->write_number( $mm,17, $row->general, $numero );		// GENERAL
					$ws->write_number( $mm,18, $row->geneimpu, $numero );		// GENEIMPU
					$ws->write_number( $mm,19, $row->adicional, $numero );	// ADICIONAL
					$ws->write_number( $mm,20, $row->adicimpu, $numero );		// ADICIMPU
					$ws->write_number( $mm,21, $row->reducida, $numero );		// REDUCIDA
					$ws->write_number( $mm,22, $row->reduimpu, $numero );		// REDUIMPU
			}
				
				$ws->write_number( $mm,23, $row->reiva, $numero );		    // IVA RETENIDO
				$ws->write_string( $mm,24, $row->comprobante, $cuerpo );	// NRO COMPROBANTE
				$fecharece = '';
				if ( !empty($row->fecharece) )
					$fecharece = substr($row->fecharece,8,2)."/".substr($row->fecharece,5,2)."/".substr($row->fecharece,0,4);
				$ws->write_string( $mm,25, $fecharece, $cuerpo );	// FECHA COMPROB
				$ws->write_string( $mm,26, $row->nfiscal, $numero );	
				$mm++;
			}
		}
		...
		$ws->write_formula( $mm,22, "=SUM(W$ii:W$mm)", $Tnumero );   //"BASE IMPONIBLE"
		$ws->write_formula( $mm,23, "=SUM(X$ii:X$mm)", $Tnumero );   //"BASE IMPONIBLE"
...
function wlvexcel2($mes) {
	...
	function wlvexcelpdvfiscal2($mes,$modalidad='M'){
		set_time_limit(300);
		$ameses = array( 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic');
		$anomeses = array( '01' => 'ENERO', '02' => 'FEBRERO', '03' => 'MARZO', '04' => 'ABRIL', '05' => 'MAYO', '06' => 'JUNIO', '07' => 'JULIO', '08' => 'AGOSTO', '09' => 'SEPTIEMBRE', '10' => 'OCTBRE', '11' => 'NOVIEMBRE', '12' => 'DICIEMBRE');
	...
	function wlvexcelpdvfiscalq1($mes){
		$this->wlvexcelpdvfiscal($mes,'Q1');
	}
	
	//libro de ventas con punto de ventas Segunda quincena
	function wlvexcelpdvfiscalq2($mes){
		$this->wlvexcelpdvfiscal($mes,'Q2');
	}
	...
	function genecompras($mes){
		...
		//SUM(IF(a.iva=0 OR b.montoiva=0,1,0)*a.importe+a.licor) AS exento, 
		//ROUND(SUM(IF(a.iva IN ("+mTASA+","+mATASA+") AND b.montoiva>0,1,0)*a.importe) *(1-b.descu/b.montotot),2) AS general,  
		//ROUND(SUM(IF(a.iva IN ("+mTASA+","+mATASA+") AND b.montoiva>0,1,0)*a.montoiva)*(1-b.descu/b.montotot),2) AS geneimpu, 
		//ROUND(SUM(IF(a.iva IN ("+mSOBRETASA+","+mASOBRETASA+") AND b.montoiva>0,1,0)*a.importe) *(1-b.descu/b.montotot),2) AS adicional,
		//ROUND(SUM(IF(a.iva IN ("+mSOBRETASA+","+mASOBRETASA+") AND b.montoiva>0,1,0)*a.montoiva)*(1-b.descu/b.montotot),2) AS adicimpu, 
		//ROUND(SUM(IF(a.iva IN ("+mTASAREDUC+","+mATASAREDUC+") AND b.montoiva>0,1,0)*a.importe) *(1-b.descu/b.montotot),2) AS reducida, 
		//ROUND(SUM(IF(a.iva IN ("+mTASAREDUC+","+mATASAREDUC+") AND b.montoiva>0,1,0)*a.montoiva)*(1-b.descu/b.montotot),2) AS reduimpu, 
		//b.montotot-b.descu+b.licor AS stotal,
		//b.montoiva AS impuesto, 
		//b.montonet-b.descu AS gtotal, 
		//b.reteiva AS reiva, "
		//TOS(BOM(mFECHA))+" AS fechal, 
		//0 AS fafecta 
		//FROM itscst AS a JOIN scst as b ON a.control=b.control  
		//LEFT JOIN sprv AS c ON b.proveed=c.proveed 
		//WHERE EXTRACT(YEAR_MONTH FROM a.fecha)="+LEFT(DTOS(mFECHA),6)+" "
		//AND b.actuali >= b.fecha 
		//GROUP BY a.control
		...
		$mSQL = "INSERT INTO siva  
			...
			ROUND(SUM((a.iva=".$mivar.")*(b.montoiva>0)*a.montoiva)*(1-b.descu/b.montotot),2)    AS reduimpu,
			ROUND(SUM((a.iva=".$mivar.")*(b.montoiva>0)*a.importe) *(1-b.descu/b.montotot),2)    AS reducida,
		...
		FROM itscst AS a JOIN scst as b ON a.control=b.control
		LEFT JOIN sprv AS c ON b.proveed=c.proveed 
		WHERE EXTRACT(YEAR_MONTH FROM b.recep)=$mes AND b.actuali >= b.fecha 
		GROUP BY b.control";
		...
		//	'CO' AS contribu, 
		//	c.rif, 
		//	if(b.fecha<'$mFECHAA' AND b.recep>='$mFECHAF' ,'04', '01') AS registro, 
		//	'S' AS nacional, 
		//	IF(b.montoiva=0,montotot,b.exento*((b.montotot-b.descu)/b.montotot))    exento,
		//	b.montasa*((b.montotot-b.descu)/b.montotot)*(b.montoiva!=0)   general,   
		//	b.tasa*((b.montotot-b.descu)/b.montotot)      geneimpu,  
		//	b.monadic*((b.montotot-b.descu)/b.montotot)   adicional, 
		//	b.sobretasa*((b.montotot-b.descu)/b.montotot) reduimpu,  
		//	b.monredu*((b.montotot-b.descu)/b.montotot)   reducida,  
		//	b.reducida*((b.montotot-b.descu)/b.montotot)  adicimpu,  
		//	b.montotot-b.descu+b.licor AS stotal,
		//	b.montoiva AS impuesto, 
		//	b.montonet-b.descu AS gtotal, 
		//	b.reteiva AS reiva, 
	...
	function geneventasfiscal($mes){
		...
		if ($query->num_rows() > 0){
			...
			foreach ($query->result() as $row){
				...
				$flag=$this->db->simple_query($mSQL);
				if(!$flag){ memowrite($mSQL,'geneventasfiscal'); return 0;};
	...
	function genesfac($mes){
		...
		$mSQL = "INSERT INTO siva  
				...
				WHERE EXTRACT(YEAR_MONTH FROM a.fecha)=$mes AND MID(numero,1,1)<>'_'";
		$flag=$this->db->simple_query($mSQL);    
	...
	function generest($mes){
		...
		if ($query->num_rows() > 0) {
			foreach ( $query->result() AS $row ){
				$mSQL = "UPDATE siva SET reiva=".$row->monto.", comprobante='20".$row->num_ref."' WHERE tipo='".$row->tipo_doc."' AND numero='".$row->numero."' AND libro='V' AND EXTRACT(YEAR_MONTH FROM fechal)=$mes ";
				$flag=$this->db->simple_query($mSQL); if(!$flag) memowrite($mSQL,'generest');
			}
		}
	...
	function genehotel($mes){
		...
		$flag=$this->db->simple_query($mSQL); if(!$flag) memowrite($mSQL,'genehotel');
	...
	function configurar(){
		...
		$uri = anchor('finanzas/libros_calore/cedit/show/<#metodo#>','<#nombre#>');
		$link=site_url('/finanzas/libros_calore');
		...
		$grid->add("finanzas/libros_calore/cedit/create");
		$grid->button('back',RAPYD_BUTTON_BACK, "javascript:window.location='$link'", "BL");
		$grid->build();
		//echo $grid->db->last_query();
		
		$link=site_url('/finanzas/libros_calore/activar');
		$data['script']='<script type="text/javascript">
		$(document).ready(function() {
	...
	function cedit(){ 
		...
		$edit->back_url = site_url("finanzas/libros_calore/configurar");
