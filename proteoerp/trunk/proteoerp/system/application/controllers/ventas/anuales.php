...
class Anuales extends Controller {
	...
	function grafico($anio=''){
		...
		for($i=0;$i<count($data_1);$i++ ){
			$mes=$i+1;      			
			$odata_1->links[]= site_url("/ventas/mensuales/index/$mes/$anio");
			$odata_2->links[]= site_url("/ventas/mensuales/index/$mes/$anio");
			$odata_3->links[]= site_url("/ventas/mensuales/index/$mes/$anio");		 	                                                                                  
		}
	...
	function _sincrodata($fechai,$fechaf){
		...
		COUNT(*) AS numfac FROM sfac WHERE tipo_doc<>'X' AND fecha>='$fechai' AND fecha<='$fechaf' GROUP BY MONTH(fecha)";
