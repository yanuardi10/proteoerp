...
class Proveedores extends Controller {  
	...
	function mensuales($anio='',$proveed=''){
		...
		if (empty($anio) OR empty($proveed)) redirect("compras/proveedores/anuales/$anio");
		...
		"SUM(montonet*IF(tipo_doc='D', -1, 1)) AS grantotal","DATE_FORMAT(fecha,'%m/%Y')AS mes",                              
	...
	function gmensuales($anio='',$proveed=''){
		...
		SUM(inicial*IF(tipo_doc='D', -1, 1))AS contado
		FROM scst
		WHERE tipo_doc<>'X' AND fecha>='$fechai' AND fecha<='$fechaf' AND proveed='$proveed'            
