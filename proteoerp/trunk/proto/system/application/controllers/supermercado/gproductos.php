...
class gproductos extends Controller {  
	...
	function gproductos() {
		...
		function index(){
		redirect('/supermercado/gproductos/anuales');
	}
		function anuales(){
		...
		$filter = new DataForm('supermercado/gproductos/anuales');
		...
		$filter->button("btnsubmit", "Buscar", form2uri(site_url('supermercado/gproductos/anuales/'),array('anio')), $position="BL");
		$filter->build_form();
    ...
    "COUNT(*) AS numfac");  
		...
		$grafico = open_flash_chart_object(680,450, site_url("supermercado/gproductos/ganuales/$anio/"));
		...
		if(empty($anio) OR empty($producto)) redirect("supermercado/gproductos/anuales/$anio");
		...
		$filter = new DataForm('supermercado/gproductos/mensuales');
		...
		$filter->button("btnsubmit", "Buscar", form2uri(site_url('supermercado/gproductos/mensuales/'),array('anio','codigo')), $position="BL");
		$filter->build_form();
    ...
    "SUM(a.venta)AS grantotal", 
		...
		$grafico = open_flash_chart_object(680,350, site_url("supermercado/gproductos/gmensuales/$anio/$producto"));
		$data['content']  =$grafico;
		$data['content'] .= $filter->output.$grid->output;
		$data["head"]     = $this->rapyd->get_head();
	...
	function diarias ($anio='',$producto='',$mes=''){
		...
		if(empty($mes))redirect("supermercado/gproductos/mensuales/$anio/$producto");
    ...
    GROUP BY a.codigo ORDER BY grantotal DESC LIMIT 10";
	  echo $mSQL;
		...
		for($i=0;$i<count($data_1);$i++ ){
		  ...
		  $bar_1->links[]= site_url("/supermercado/gproductos/mensuales/$anio/".str_replace('/',':slash:',$producto[$i]));			 	                                                                                  
		} 			 
			$g = new graph();
		...
		$g->set_x_legend('gproductos', 14, '#004381' );        
		
		$g->bg_colour = '#FFFFFF';
		$g->set_tool_tip( '#key#<br>Producto: #x_label# <br>Monto: #tip#');
		$g->set_y_max(ceil($maxval/$om));
		$g->y_label_steps(5);
		$g->set_y_legend('P-roducto x '.number_format($om,0,'','.').' (Bs)', 16, '#004381' );
		}else
    ...
    GROUP BY mes ORDER BY fecha";
	 ...
	 foreach($query->result() as $row ){ if ($row->grantotal>$maxval) $maxval=$row->grantotal;
		...
		for($i=0;$i<count($data_1);$i++ ){
			$mes=$i+1;
			$bar_1->add_data_tip($data_1[$i]/$om, graph::esc( number_format($data_1[$i],2,',','.')));  
		  $bar_1->links[]= site_url("/supermercado/gproductos/diarias/$anio/$producto/$mes");			 	                                                                                  
		} //$bar_1->links[]= site_url("/supermercado/gproductos/mensuales/$anio/".$producto[$i]);				 
		
		$g = new graph();  
		$g->set_is_decimal_separator_comma(1);
		if($maxval>0){
		$g->title( 'supermercado de '.$nombre.' en el a&ntilde;o '.$anio,'{font-size: 16px; color:#0F3054}' );
		$g->data_sets[] = $bar_1;
