...
class Chofer extends Controller {
	...
	var $titp    = 'CHOFERES';
	var $tits    = 'CHOFERES';
	var $url     = 'ventas/chofer/';

	function Chofer(){
		parent::Controller();
		$this->load->library('rapyd');
		$this->load->library('jqdatagrid');
		$this->datasis->modulo_nombre( 'CHOFER', $ventana=0 );
	}
	...
	function jqdatag(){
		...
		$param['listados']    = $this->datasis->listados('CHOFER', 'JQ');
		$param['otros']       = $this->datasis->otros('CHOFER', 'JQ');
						...
						success: function(r,s,x){
							try{
								...
								if (json.status == "A"){
									...
									'.$this->datasis->jwinopen(site_url('formatos/ver/CHOFER').'/\'+res.id+\'/id\'').';
									return true;
								} else {
	...
	function defgrid( $deployed = false ){
		...
		$grid->setAdd(    $this->datasis->sidapuede('CHOFER','INCLUIR%' ));
		$grid->setEdit(   $this->datasis->sidapuede('CHOFER','MODIFICA%'));
		$grid->setDelete( $this->datasis->sidapuede('CHOFER','BORR_REG%'));
		$grid->setSearch( $this->datasis->sidapuede('CHOFER','BUSQUEDA%'));
		$grid->setRowNum(30);
	...
	*/
	function getdata(){
		...
		$mWHERE = $grid->geneTopWhere('chofer');

		$response   = $grid->getData('chofer', array(array()), array(), false, $mWHERE );
	...
	*/
	function setData(){
		...
		if($oper == 'add'){
			if(false == empty($data)){
				$check = $this->datasis->dameval("SELECT count(*) FROM chofer WHERE $mcodp=".$this->db->escape($data[$mcodp]));
				if ( $check == 0 ){
					$this->db->insert('chofer', $data);
					echo "Registro Agregado";

					logusu('CHOFER',"Registro ????? INCLUIDO");
				} else
			...
			$nuevo  = $data[$mcodp];
			$anterior = $this->datasis->dameval("SELECT $mcodp FROM chofer WHERE id=$id");
			if ( $nuevo <> $anterior ){
				//si no son iguales borra el que existe y cambia
				$this->db->query("DELETE FROM chofer WHERE $mcodp=?", array($mcodp));
				$this->db->query("UPDATE chofer SET $mcodp=? WHERE $mcodp=?", array( $nuevo, $anterior ));
				$this->db->where("id", $id);
				$this->db->update("chofer", $data);
				logusu('CHOFER',"$mcodp Cambiado/Fusionado Nuevo:".$nuevo." Anterior: ".$anterior." MODIFICADO");
				echo "Grupo Cambiado/Fusionado en clientes";
			} else {
				unset($data[$mcodp]);
				$this->db->where("id", $id);
				$this->db->update('chofer', $data);
				logusu('CHOFER',"Grupo de Cliente  ".$nuevo." MODIFICADO");
				echo "$mcodp Modificado";
			}

		} elseif($oper == 'del') {
			$meco = $this->datasis->dameval("SELECT $mcodp FROM chofer WHERE id=$id");
			//$check =  $this->datasis->dameval("SELECT COUNT(*) FROM chofer WHERE id='$id' ");
			if ($check > 0){
				echo " El registro no puede ser eliminado; tiene movimiento ";
			} else {
				$this->db->simple_query("DELETE FROM chofer WHERE id=$id ");
				logusu('CHOFER',"Registro ????? ELIMINADO");
