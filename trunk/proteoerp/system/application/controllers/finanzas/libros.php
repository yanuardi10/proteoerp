	...
	function wlvexcel($mes){
		...
		if ( $export->num_rows() > 0 ){
			foreach( $export->result() as $row ){
				...
				$ws->write_number( $mm,18, $row->reiva, $numero );		    // IVA RETENIDO
				$ws->write_string( $mm,19, $row->comprobante, $cuerpo );	// NRO COMPROBANTE
				$ws->write_string( $mm,22, $row->referen, $numero ); //Nº FACT AFECTA
				$fecharece = '';
				if ( !empty($row->fecharece) )
					$fecharece = substr($row->fecharece,8,2)."/".substr($row->fecharece,5,2)."/".substr($row->fecharece,0,4);
				$ws->write_string( $mm,20, $fecharece, $cuerpo );	// FECHA COMPROB
	...
	function genesfaccierrez($mes){
		...
		if ($query->num_rows() > 0){
			...
			foreach ($query->result() as $row){
				...
				$data['exento']   =$row->exento+$row->ncexento-$tt['exento']   -$row->ncexento;
				$data['general']  =$row->base  +$row->ncbase  -$tt['general']  -$row->ncbase  ;
				$data['geneimpu'] =$row->iva   +$row->nciva   -$tt['geneimpu'] -$row->nciva   ;
				$data['adicional']=$row->base2 +$row->ncbase2 -$tt['adicional']-$row->ncbase2 ;
				$data['adicimpu'] =$row->iva2  +$row->nciva2  -$tt['adicimpu'] -$row->nciva2  ;
				$data['reducida'] =$row->base1 +$row->ncbase1 -$tt['reducida'] -$row->ncbase1 ;
				$data['reduimpu'] =$row->iva1  +$row->nciva1  -$tt['reduimpu'] -$row->nciva1  ;
