...
class Libros extends Controller {
	...
	//***********************************************
	// LIBROS Excell  NOOOOOOOOOOOOOOOOOOOOOOO
	//***********************************************
	//Libro de compras contribuyente normal
	function wlcexcelNOOOO($mes){
		$mes  = $this->uri->segment(4).$this->uri->segment(5);
		//$ano = $this->uri->segment(3);
		set_time_limit(300);
		$ameses = array( 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic');
		...
		*/
		...
		if ( $export->num_rows() > 0 ){
			foreach( $export->result() as $row ){
				...
				$ws->write_string( $mm, 3, $row->numero, $cuerpo );	// INICIAL
				if ($row->tipo[0] == "X" ) 
					$ws->write_string( $mm, 5, "Documento Anulado", $cuerpo );			// NOMBRE
				else
					$ws->write_string( $mm, 5, $row->nombre, $cuerpo );			// NOMBRE
				$ws->write_string( $mm, 6, $row->rif, $cuerpo );			// CONTRIBUYENTE
				
				if ( $row->registro=='04' ) {
					$ws->write_number( $mm, 7, 0, $numero );		// VENTAS + IVA
					$ws->write_number( $mm, 9, 0, $numero );		// VENTAS EXENTAS
					$ws->write_number( $mm,9, 0, $cuerpo );		// EXPORTACION
					$ws->write_number( $mm,10, 0, $numero );		// GENERAL
					$ws->write_number( $mm,11, 0, $numero );		// GENEIMPU
					$ws->write_number( $mm,12, 0, $numero );		// ADICIONAL
					$ws->write_number( $mm,13, 0, $numero );		// ADICIMPU
					$ws->write_number( $mm,14, 0, $numero );		// REDUCIDA
					$ws->write_number( $mm,15, 0, $numero );		// REDUIMPU
					$ws->write_number( $mm,16, $row->geneimpu + $row->adicimpu+$row->reduimpu, $numero );		// REDUIMPU
				} else {
					$ws->write_number( $mm, 7, $row->ventatotal, $numero );	// VENTAS + IVA
					$ws->write_number( $mm, 8, $row->exento, $numero );   	// VENTAS EXENTAS
					$ws->write_number( $mm, 9, 0, $cuerpo );   					    // EXPORTACION
					$ws->write_number( $mm,10, $row->general, $numero );		// GENERAL
					$ws->write_number( $mm,11, $row->geneimpu, $numero );		// GENEIMPU
					$ws->write_number( $mm,12, $row->adicional, $numero );	// ADICIONAL
					$ws->write_number( $mm,13, $row->adicimpu, $numero );		// ADICIMPU
					$ws->write_number( $mm,14, $row->reducida, $numero );		// REDUCIDA
					$ws->write_number( $mm,15, $row->reduimpu, $numero );		// REDUIMPU
				...
				$ws->write_string( $mm,20, $row->nfiscal, $numero );	
				$mm++;
			}
		}
	...
	function genecompras($mes){
		...
		// Procesando Compras scst
		$flag=var_dum($this->db->simple_query($mSQL));
	...
	function geneventasfiscalpdv($mes){
		...
		FROM fiscalz WHERE fecha BETWEEN $fdesde  AND $fhasta GROUP BY caja,fecha";
		...
		if ($query->num_rows() > 0){
			...
			foreach ($query->result() as $row){
				...
				FROM siva WHERE caja='{$row->caja}' AND fecha BETWEEN '$row->fecha1' AND '$row->fecha' AND hora>='$hdesde' AND hora<'$hhasta' AND tipo<>'CZ'
				GROUP BY tipo";
	...
	function generest($mes){
		...
		if ($query->num_rows() > 0) {
			foreach ( $query->result() AS $row ){
				$mSQL = "UPDATE siva SET reiva=".$row->monto.", comprobante='20".$row->num_ref."' WHERE tipo='".$row->tipo_doc."' AND numero='".$row->numero."' AND libro='V' AND EXTRACT(YEAR_MONTH FROM fechal)=$mes ";
				$flag=var_dum($this->db->simple_query($mSQL)); if(!$flag) memowrite($mSQL,'generest');
			}
		}
	...
	function genehotel($mes){
		...
		$flag=var_dum($this->db->simple_query($mSQL)); if(!$flag) memowrite($mSQL,'genehotel');
