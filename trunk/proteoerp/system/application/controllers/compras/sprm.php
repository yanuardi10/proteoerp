...
class Sprm extends Controller {
	var $mModulo='SPRM';
	var $titp='Cuentas por Pagar';
	var $tits='Modulo SPRM';
	var $url ='compras/sprm/';
	...
	function index(){
		/*if ( !$this->datasis->iscampo('sprm','id') ) {
			...
			$this->db->simple_query('ALTER TABLE sprm ADD UNIQUE INDEX numero (numero)');
...
jQuery("#a1").click( function(){
	...
	if (id)	{
		...
		window.open(\''.base_url().'formatos/ver/SPRM/\'+id, \'_blank\', \'width=800,height=600,scrollbars=yes,status=yes,resizable=yes,screenx=((screen.availHeight/2)-400), screeny=((screen.availWidth/2)-300)\');
...
';
	...
	*/
	function getdata()
	{
		...
		$response   = $grid->getData('sprm', array(array()), array(), false, $mWHERE );
	...
	*/
	function setData()
	{
		...
		if($oper == 'add'){
			if(false == empty($data)){
				$check = $this->datasis->dameval("SELECT count(*) FROM sprm WHERE $mcodp=".$this->db->escape($data[$mcodp]));
				if ( $check == 0 ){
					$this->db->insert('sprm', $data);
					echo "Registro Agregado";

					logusu('SPRM',"Registro ????? INCLUIDO");
				} else
			...
			$nuevo  = $data[$mcodp];
			$anterior = $this->datasis->dameval("SELECT $mcodp FROM sprm WHERE id=$id");
			if ( $nuevo <> $anterior ){
				//si no son iguales borra el que existe y cambia
				$this->db->query("DELETE FROM sprm WHERE $mcodp=?", array($mcodp));
				$this->db->query("UPDATE sprm SET $mcodp=? WHERE $mcodp=?", array( $nuevo, $anterior ));
				$this->db->where("id", $id);
				$this->db->update("sprm", $data);
				logusu('SPRM',"$mcodp Cambiado/Fusionado Nuevo:".$nuevo." Anterior: ".$anterior." MODIFICADO");
				echo "Grupo Cambiado/Fusionado en clientes";
			} else {
				unset($data[$mcodp]);
				$this->db->where("id", $id);
				$this->db->update('sprm', $data);
				logusu('SPRM',"Grupo de Cliente  ".$nuevo." MODIFICADO");
				echo "$mcodp Modificado";
			}

		} elseif($oper == 'del') {
		$meco = $this->datasis->dameval("SELECT $mcodp FROM sprm WHERE id=$id");
