	...
	function Table(){
		...
		foreach( $res->result_array() as $row ){
			...
			foreach( $this->aCols  as $i=>$fila ){
				...
				if    ($this->DBfieldsType[$key]=='real' || in_array($this->DBfieldsType[$key], array(4,5,246))  ) $row[$key]=nformat($row[$key]);
				elseif($this->DBfieldsType[$key]=='date' || in_array($this->DBfieldsType[$key], array(10,12,7))  ) $row[$key]=dbdate_to_human($row[$key]);
				elseif($this->DBfieldsType[$key]=='int'  || in_array($this->DBfieldsType[$key], array(1,2,9,3,8))) $row[$key]=intval($row[$key]);
			}
			$this->Row($row);
		}
