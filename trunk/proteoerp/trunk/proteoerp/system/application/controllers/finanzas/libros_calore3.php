...
class Libros_calore3 extends Controller {

	function Libros_calore3() {
		parent::Controller();
	...
	function index() {
		...
		$link=site_url('finanzas/libros_calore3/generar');
		...
		function descarga(nombre){
			...
			window.location="'.site_url('finanzas/libros_calore3').'/"+nombre+"/"+param;
			//alert(param);
		}
		</script>';
		$conf=anchor('finanzas/libros_calore3/configurar','Configurar');
		$form=  form_open('/finanzas/libros_calore3');
...
function wlvexcel2($mes) {
	...
	function geneventasfiscal($mes){
		...
		$fdesde=$mes.'01';
		$fhasta=$mes.$udia;
		//Por si no se hiso el cierre al final del mes pasado
		$fdesde=$this->datasis->dameval("SELECT MIN(fecha1) FROM fiscalz WHERE fecha BETWEEN $fdesde AND $fhasta");
		...
		if ($query->num_rows() > 0){
			...
			foreach ($query->result() as $row){
				...
				LEFT JOIN club c ON b.cliente=c.cod_tar 
				LEFT JOIN dine d ON a.fecha=d.fecha AND a.caja=d.caja AND a.cajero=d.cajero
				WHERE a.fecha >='$row->fecha1' AND a.fecha<='$row->fecha' AND c.cedula REGEXP '^[VEJG][0-9]{9}$' 
	...
	function configurar(){
		...
		$uri = anchor('finanzas/libros_calore3/cedit/show/<#metodo#>','<#nombre#>');
		$link=site_url('/finanzas/libros_calore3');
		...
		$grid->add("finanzas/libros_calore3/cedit/create");
		$grid->button('back',RAPYD_BUTTON_BACK, "javascript:window.location='$link'", "BL");
		$grid->build();
		//echo $grid->db->last_query();
		
		$link=site_url('/finanzas/libros_calore3/activar');
		$data['script']='<script type="text/javascript">
		$(document).ready(function() {
	...
	function cedit(){ 
		...
		$edit->back_url = site_url("finanzas/libros_calore3/configurar");
