<?php
//$this->db->simple_query("UPDATE formatos SET proteo=NULL WHERE nombre='RETC'");
if(count($parametros)==0) show_error('Faltan parametros ');
$id=$parametros[0];
$dbid=$this->db->escape($id);
$dbid=$id;
$mSQL_1 = $this->db->query("SELECT emision, fecha, cod_cli, nombre, rif, stotal, impuesto, gtotal, monto, estampa, hora, usuario, modificado, transac, sprmreinte, operacion,codbanc FROM retc WHERE id=${dbid}");
if($mSQL_1->num_rows()==0) show_error('Registro no encontrado');
$mSQL_2 = $this->db->query("SELECT tipo_doc,fecha,numero,nfiscal,impuesto,gtotal,monto,transac,estampa,hora,usuario,ffactura,modificado FROM itretc WHERE idretc=${dbid}");
...
WHERE b.codbanc=".$this->db->escape($codbanc).' AND b.transac='.$dbtransac;
...
$mSQL_3 = $this->db->query("SELECT SUM(monto*IF(tipo_doc IN ('AN','NC'),1,-1)) AS anticipo,GROUP_CONCAT(numero) AS numero FROM smov WHERE transac=${dbtransac} AND tipo_doc IN ('AN','ND') AND cod_cli=".$this->db->escape($cod_cli));
$rrow = $mSQL_3->row();
...
<title>Consignacion de Comprobante de retencion ISLR</title>
...
<table style="width: 100%;">
	...
	<tr>
		<td><div id="section_header">
			...
			<tr>
				<td valign='bottom'><h1 style="text-align: left" >Consignaci&oacute;n de Retenci&oacute;n de ISLR</h1></td>
	...
	<tr>
		<td style="text-align:center;">Hemos recibido del Agente de Retencion de ISLR el documento especificado a continuaci&oacute;n</td>
	</tr>
	<tr>
		<td>
	...
	<tr>
		...
		<tbody>
			<?php $tmonto=0;  $mod=false; $i=0; foreach ($detalle as $items){ $i++;?>
			<?php $tmonto+=$items->monto?>
		...
		<tr>
			...
			<td class="change_order_total_col" style="font-size:14pt;border-style:solid;"><b><?php echo nformat($tmonto) ?></b></td>
		</tr>
		<tr>
