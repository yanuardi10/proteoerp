...
$mSQL = 'SELECT
a.numero,a.fecha,almacen,CONCAT_WS("",TRIM(observ1),observ2) AS observa
,b.ubides AS almacendes
FROM conv AS a
...
a.codigo,b.descrip AS desca,a.salida,a.entrada
FROM itconv AS a
...
<title>CONVERSI&Oacute;N <?php echo $numero ?></title>
...
$encabezado = "
	<table style='width:100%;font-size: 9pt;' class='header' cellpadding='0' cellspacing='0'>
		<tr>
			<td valign='bottom'><h1 style='text-align:left;border-bottom:1px solid;font-size:12pt;'>CONVERSI&Oacute;N Nro. ${numero}</h1></td>
			<td valign='bottom'><h1 style='text-align:right;border-bottom:1px solid;font-size:12pt;'>Fecha: ${fecha}</h1></td>
		</tr><tr>
			<td colspan='2'>Almac&eacute;n:<b>(${almacen}) ${almacendes}</b></td>
		</tr>
		<tr>
...
?>
			<tr class="<?php if(!$mod) echo 'even_row'; else  echo 'odd_row'; ?>">
				...
				<td style="text-align: right;"><?php echo ($clinea)? '': nformat($items->salida); ?></td>
				<td style="text-align: right;"><?php echo ($clinea)? '': nformat($items->entrada); ?></td>
			</tr>
<?php
		if($npagina){
			echo $pie_continuo;
